package com.aizhixin.cloud.mobile.common.util;
...
public class RestUtil {
    ...
    private HttpHeaders getHeader(String authorization) {
        HttpHeaders header = getAuthorizationHeaders(authorization);
...
//        System.out.println("------------------url:" + url + "\t" + method.name());
    ...
    public String oauthLogin(String auth, String url, String username, String password) {
        return httpRequest(url, getLoginEntiry(getAuthorizationHeaders(auth),username, password), HttpMethod.POST).getBody();
    }
    public String get(String url, String authorization) {
        HttpEntity<String> entity = new HttpEntity<>(getHeader(authorization));
    ...
    public String postBody(String url, String json, String authorization) {
        HttpEntity<byte[]> entity = new HttpEntity<>(json.getBytes(Charset.forName("UTF-8")), getHeader(authorization));
        return httpRequest(url, entity, HttpMethod.POST).getBody();
    }
    public String postBody(String url, Set<Long> set, String authorization) {
        HttpEntity<Set<Long>> entity = new HttpEntity<>(set,getHeader(authorization));
        return httpRequest(url, entity, HttpMethod.POST).getBody();
    }
    public String postStringListBody(String url, Set<String> set, String authorization) {
        HttpEntity<Set<String>> entity = new HttpEntity<>(set,getHeader(authorization));
        return httpRequest(url, entity, HttpMethod.POST).getBody();
    }

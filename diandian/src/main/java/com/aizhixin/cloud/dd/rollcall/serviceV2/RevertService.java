...
public class RevertService {
	...
	public Revert save(RevertDTO revertDTO,AccountDTO accountDTO) {
		...
		if(null!=rt.getToUserId()){
			PushMessage pm = pms.createPushMessage("", revertDTO.getFromUserName() + "回复你：" + revertDTO.getContent(), "revert_notice", "revert", "回复通知", rt.getToUserId());
			//----新消息服务----start
			...
			dto.setData(rt);
			audiences.add(dto);
			messageService.push("回复通知", revertDTO.getFromUserName() + "回复你：" + revertDTO.getContent(), "revert_notice", audiences);
			//----新消息服务----end
		...
		if (null != as) {
			...
			if(null==revertDTO.getToUserId()||0l==revertDTO.getToUserId()){
				...
				PushMessage pm = pms.createPushMessage("", revertDTO.getFromUserName() + "回复你：" + revertDTO.getContent(), "revert_notice", "revert", "回复通知", as.getCommentId());
				//----新消息服务----start
				List<AudienceDTO> audiences = new ArrayList<>();
				AudienceDTO dto = new AudienceDTO();

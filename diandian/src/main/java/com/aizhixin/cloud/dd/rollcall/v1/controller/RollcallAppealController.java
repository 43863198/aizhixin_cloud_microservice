...
import com.aizhixin.cloud.dd.rollcall.service.RollcallAppealService;
...
@RestController
@RequestMapping("/api/phone/v1/appeal")
@Api(description = "考勤申诉")
public class RollcallAppealController {

    @Autowired
    private RollcallAppealService appealService;
    @Autowired
    ...
    public ResponseEntity<?> create(@RequestHeader("Authorization") String accessToken,
                                    @ApiParam(value = "申诉信息", required = true) @RequestBody RollCallAppealDTO appealDTO) {
        AccountDTO account = ddUserService.getUserInfoWithLogin(accessToken);
        if (account == null) {
            return new ResponseEntity<Object>(TokenUtil.tokenValid(), HttpStatus.UNAUTHORIZED);
        }
        Map<String, Object> result = appealService.create(accessToken, account.getId(), appealDTO);
    ...
    public ResponseEntity<?> getListByStudent(@RequestHeader("Authorization") String accessToken,
        ...
        PageData<RollCallAppealDomain> result = appealService.getListByStudent(account.getId(), pageable, appealStatus);
        return new ResponseEntity(result, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> getListByTeacher(@RequestHeader("Authorization") String accessToken,
        ...
        Pageable pageable = PageUtil.createNoErrorPageRequestAndSortType(pageNumber, pageSize, "DESC", "id");
        PageData<RollCallAppealDomain> result = appealService.getListByTeacher(account.getId(), pageable, appealStatus);
        return new ResponseEntity(result, HttpStatus.OK);
    }

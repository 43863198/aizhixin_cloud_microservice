...
public class AttendanceRecordService {
    ...
    public PageData<AttendanceRecordDTO> searchAttendance(Long orgId, Long collegeId, String criteria, String startTime, String endTime, String teachingClassName, String teacherName, String courseName, Integer type, Pageable pageable) {
        ...
        try {
            ...
            StringBuilder cql = new StringBuilder("SELECT r.rcount FROM (SELECT count(1) AS rcount, rc.ID as id FROM dd_rollcall rc LEFT JOIN dd_schedule_rollcall sr ON rc.SCHEDULE_ROLLCALL_ID = sr.ID LEFT JOIN dd_schedule s ON sr.SCHEDULE_ID = s.ID WHERE rc.delete_flag=0 ");
            ...
            sql.append("SELECT rc.STUDENT_NUM, rc.STUDENT_NAME, s.COURSE_NAME, s.TEACHER_NAME, s.TEACH_DATE, s.START_TIME, s.END_TIME, s.CLASSROOM_NAME, rc.TYPE, rc.SIGN_TIME, rc.DISTANCE, rc.ID AS id, rc.CREATED_DATE FROM " +
                    "dd_rollcall rc LEFT JOIN dd_schedule_rollcall sr ON rc.SCHEDULE_ROLLCALL_ID = sr.ID LEFT JOIN dd_schedule s ON sr.SCHEDULE_ID = s.ID WHERE rc.delete_flag=0 ");
            if (null != orgId) {

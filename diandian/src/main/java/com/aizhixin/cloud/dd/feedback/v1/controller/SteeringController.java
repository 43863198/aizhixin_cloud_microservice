 ...
 * @author hsh
 */
@RequestMapping("/api/phone/v1/feedback/steering")
@RestController
@Api(value = "督导反馈相关API", description = "督导反馈相关API")
public class SteeringController {
     ...
     * 督导员获取督导反馈列表
     */
    @RequestMapping(value = "/supervisor/getFeelbackList", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(httpMethod = "GET", value = "督导员获取督导反馈列表", response = Void.class, notes = "督导员获取督导反馈列表<br>@author hsh")
    public ResponseEntity<?> getStuFeelbackList(
        ...
        PageData<FeedbackRecordDomain> result = recordService.queryListByJobNum(pageable, account.getOrganId(), account.getPersonId(), FeedbackTempletType.STEERING.getType());
    ...
    @ApiOperation(httpMethod = "GET", value = "获取督导反馈列表", response = Void.class, notes = "获取督导反馈列表<br>@author hsh")
    public ResponseEntity<?> getFeelbackList(
            @RequestHeader("Authorization") String accessToken,
            @ApiParam(value = "orgId") @RequestParam(value = "orgId", required = true) Long orgId,
            @ApiParam(value = "督导员名或者工号") @RequestParam(value = "supName", required = false) String supName,
        ...
        Pageable pageable = PageUtil.createNoErrorPageRequest(pageNumber, pageSize);
        PageData<FeedbackRecordDomain> result = recordService.queryList(pageable, orgId, supName, teacherName, courseName, FeedbackTempletType.STEERING.getType());
        return new ResponseEntity(result, HttpStatus.OK);
    }
    ...
    @ApiOperation(httpMethod = "GET", value = "获取督导反馈", response = Void.class, notes = "获取督导反馈<br>@author hsh")
    ...
    @ApiOperation(httpMethod = "POST", value = "保存督导反馈", response = Void.class, notes = "保存督导反馈<br>@author hsh")
    public ResponseEntity<?> saveFeedback(@RequestHeader("Authorization") String accessToken, @ApiParam(value = "模板") @RequestBody FeedbackRecordDTO feedback) {
        ...
        try {
            feedback.setType(FeedbackTempletType.STEERING.getType());
    ...
    @ApiOperation(httpMethod = "GET", value = "导出督导反馈列表", response = Void.class, notes = "导出督导反馈列表<br>@author hsh")
    public ResponseEntity<?> exportFeelbackList(
        ...
        Pageable pageable = PageUtil.createNoErrorPageRequest(pageNumber, pageSize);
        String filename = "督导反馈.xlsx";
        String url = recordService.exportList(pageable, orgId, supName, teacherName, courseName, FeedbackTempletType.STEERING.getType(), filename);
        Map<String, Object> result = new HashMap<>();
        if (url != null) {

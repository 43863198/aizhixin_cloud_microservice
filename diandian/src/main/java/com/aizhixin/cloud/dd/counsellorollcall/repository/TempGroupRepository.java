...
public interface TempGroupRepository extends JpaRepository<TempGroup, Long> {
    ...
    @Query("select new com.aizhixin.cloud.dd.counsellorollcall.domain.TempGroupDomainV2(g.id,g.name,g.subGroupNum,g.rollcallNum,g.status,g.rollcallType,g.ruleId, g.messageId) from #{#entityName} g where g.teacherId = :teacherId and g.deleteFlag = :deleteFlag")
    ...
    @Query("select new com.aizhixin.cloud.dd.counsellorollcall.domain.TempGroupDomainV2(g.id,g.name,g.subGroupNum,g.rollcallNum,g.status,g.rollcallType,g.ruleId, g.messageId) from #{#entityName} g where g.teacherId = :teacherId and g.practiceId=:practiceId and g.deleteFlag = :deleteFlag")
    public List<TempGroupDomainV2> findByTeacherIdAndPracticeIdAndDeleteFlag(@Param(value = "teacherId") Long teacherId, @Param(value = "practiceId") Long practiceId, @Param(value = "deleteFlag") Integer deleteFlag);
    ...
    @Query("select new com.aizhixin.cloud.dd.counsellorollcall.domain.TempGroupDomainV2(g.id,g.name,g.subGroupNum,g.rollcallNum,g.status,g.rollcallType,g.ruleId, g.messageId) from #{#entityName} g where g.practiceId=:practiceId and g.deleteFlag = :deleteFlag")
    ...
    public List<TempGroup> findAllByStatus(Boolean status);

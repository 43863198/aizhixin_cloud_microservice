...
public interface LeaveRepository extends JpaRepository<Leave, Long>, JpaSpecificationExecutor<Leave> {
    ...
    public List<Leave> findByDeleteFlagAndStartTimeIsNull(Integer deleteFlage);

    @Query(value = "select new com.aizhixin.cloud.dd.rollcall.domain.LeaveDomain(id, studentId, studentName, headTeacherId, teacherName, startTime, endTime, className, requestContent, status, leavePublic, leaveType) FROM #{#entityName} t where t.status=:status and t.leavePublic=:leavePublic and t.leaveType=:leaveType and t.deleteFlag=:deleteFlag and (t.studentName LIKE CONCAT('%',:studentName,'%') or t.studentJobNum LIKE CONCAT('%',:studentName,'%')) and (t.teacherName LIKE CONCAT('%',:teacherName,'%') or t.teacherJobNum LIKE CONCAT('%',:teacherName,'%')) and t.className LIKE CONCAT('%',:className,'%')")
    public Page<LeaveDomain> findByStatusAndLeavePublicAndLeaveTypeAndDeleteFlagAndNameLike(Pageable pageable, @Param("status") String status, @Param("leavePublic") Integer leavePublic, @Param("leaveType") Integer leaveType, @Param("deleteFlag") Integer deleteFlag, @Param("studentName") String studentName, @Param("teacherName") String teacherName, @Param("className") String className);
    ...
    public Page<LeaveDomain> findByStatusAndLeavePublicAndDeleteFlagAndNameLike(Pageable pageable, @Param("status") String status, @Param("leavePublic") Integer leavePublic, @Param("deleteFlag") Integer deleteFlag, @Param("studentName") String studentName, @Param("teacherName") String teacherName, @Param("className") String className);

    @Query(value = "select new com.aizhixin.cloud.dd.rollcall.domain.LeaveDomain(id, studentId, studentName, headTeacherId, teacherName, startTime, endTime, className, requestContent, status, leavePublic, leaveType) FROM #{#entityName} t where t.status=:status and t.deleteFlag=:deleteFlag and (t.studentName LIKE CONCAT('%',:studentName,'%') or t.studentJobNum LIKE CONCAT('%',:studentName,'%')) and (t.teacherName LIKE CONCAT('%',:teacherName,'%') or t.teacherJobNum LIKE CONCAT('%',:teacherName,'%')) and t.className LIKE CONCAT('%',:className,'%')")
    public Page<LeaveDomain> findByStatusAndDeleteFlagAndNameLike(Pageable pageable, @Param("status") String status, @Param("deleteFlag") Integer deleteFlag, @Param("studentName") String studentName, @Param("teacherName") String teacherName, @Param("className") String className);

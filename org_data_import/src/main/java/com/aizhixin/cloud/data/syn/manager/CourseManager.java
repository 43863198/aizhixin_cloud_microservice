...
public class CourseManager implements BaseDataManager {
	private final static String KEY = "KCDM";// 数据库字段名称，课程编码
	private final static String NAME = "KCZWMC";// 数据库字段名称，课程中文名称
	private final static String KCXZ = "KCXZ";// 数据库字段名称，课程性质
	...
	public List<BaseDTO> findAll() {
		...
		for (Map<String, Object> m : ds) {
			...
			if (!StringUtils.isEmpty(m.get(KCXZ))) {
				d.setKcxz(m.get(KCXZ).toString());
			}
			list.add(d);
		}
		return list;

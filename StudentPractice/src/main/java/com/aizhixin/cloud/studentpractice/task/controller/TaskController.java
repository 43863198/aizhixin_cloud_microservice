...
public class TaskController {
	...
	public ResponseEntity<Map<String, Object>> add(
		...
		if (roles.contains(RoleCode.ROLE_COM_TEACHER)) {
			...
			List<IdNameDomain> groupList = authUtilService
					.getMentorGroupInfo(dto.getId());
			if (groupList.size() > 0) {
	...
	private ResponseEntity<Map<String, Object>> findMentorTaskPage(
			...
			AccountDTO dto) {
		PageData pageData = mentorTaskService.findMenTaskPage(
				domain.getPageSize(), domain.getPageNumber(),
				domain.getTaskName(), domain.getProgress(), dto.getId(),
				domain.getWeekTaskId(), domain.getSortFlag());
		...
		if (null != dataList && !dataList.isEmpty()) {
			...
			HashMap<String, MentorTaskCount> countMap = mentorTaskCountService
					.findByMentorTaskIds(ids);
			for (TaskDomain taskDomain : dataList) {
				if (null != countMap.get(taskDomain.getId())) {
					MentorTaskCount countDomain = countMap.get(taskDomain
							.getId());
					taskDomain.setFinishNum(countDomain.getFinishNum());
					taskDomain.setCheckPendingNum(countDomain
							.getCheckPendingNum());
				}
			}
		}
	...
	private ResponseEntity<Map<String, Object>> findMentorTaskPageByGroup(
			QueryTaskPageDomain domain, Map<String, Object> result,
			String groupId) {
		PageData<TaskDomain> pageData = mentorTaskService
				.findMenTaskPageByGroupId(domain.getPageSize(),
						domain.getPageNumber(), domain.getTaskName(),
						domain.getProgress(), groupId, domain.getSortFlag());
		List<TaskDomain> dataList = pageData.getData();
		if (null != dataList && !dataList.isEmpty()) {
			Set<String> ids = new HashSet<String>();
			for (TaskDomain taskDomain : dataList) {
	...
	public ResponseEntity<Map<String, Object>> stuTaskPage(
			...
			if (roles.contains(RoleCode.ROLE_STUDENT)) {
				...
				if (null == groupId) {
					...
					return new ResponseEntity<Map<String, Object>>(
							stuTaskService.findStuTaskPageByGroupId(
									domain.getPageSize(),
									domain.getPageNumber(),
									domain.getTaskName(),
									domain.getTaskStatus(), dto.getId(),
									groupId, domain.getSortFlag()),
							HttpStatus.OK);
				}
			}
		}
		...
		return new ResponseEntity<Map<String, Object>>(
				stuTaskService.findStuTaskPage(domain.getPageSize(),
						domain.getPageNumber(), domain.getTaskName(),
						domain.getTaskStatus(), dto.getId(),
						domain.getWeekTaskId(), domain.getSortFlag()),
				HttpStatus.OK);
	...
	public ResponseEntity<Map<String, Object>> mentorEditTaskDetail(
		...
		MentorTaskDetailDomain domain = mentorTaskService
				.getMentorEditTaskDetail(mentorTaskId, dto);
	...
	public ResponseEntity<Map<String, Object>> mentorCompleteTaskDetail(
		...
		MentorTaskDetailDomain domain = mentorTaskService
				.getMentorTaskDetail(mentorTaskId);
		result.put(ApiReturnConstants.DATA, domain);
		return new ResponseEntity<Map<String, Object>>(result, HttpStatus.OK);
	}
	...
	public StudentTask stuBack(
		...
		if (TaskStatusCode.TASK_STATUS_CHECK_PENDING.equals(studentTask
				.getStudentTaskStatus())) {
			studentTask
					.setStudentTaskStatus(TaskStatusCode.TASK_STATUS_UNCOMMIT);
	...
	public ResponseEntity<Map<String, Object>> taskCount(
		...
		result.put(ApiReturnConstants.DATA,
				stuTaskService.countStuTaskStatusNum(dto.getId()));
		return new ResponseEntity<Map<String, Object>>(result, HttpStatus.OK);
	}
	...
	public ResponseEntity<Map<String, Object>> summaryCount(
		...
		result.put(ApiReturnConstants.DATA,
				stuTaskService.countTaskStatusNum(dto.getId()));
		return new ResponseEntity<Map<String, Object>>(result, HttpStatus.OK);
	}
	...
	public ResponseEntity<Map<String, Object>> review(
		...
		if (!TaskStatusCode.TASK_STATUS_BACK_TO
				.equals(domain.getReviewResult())
				&& StringUtils.isEmpty(domain.getReviewScore())) {
			throw new CommonException(ErrorCode.ID_IS_REQUIRED,
					"学生实践任务评审分数不能为空");
		}
		// domain.setCreatorName(dto.getName());
		ReviewTask reviewTask = reviewTaskService.review(domain, dto.getId(),
				token);
	...
	public ResponseEntity<Map<String, Object>> stuInfoPage(
			@RequestHeader("Authorization") String token,
			// @ApiParam(value = "stuName 学生名称") @RequestParam(value =
			// "stuName", required = false) String stuName,
			// @ApiParam(value = "pageNumber 第几页") @RequestParam(value =
			// "pageNumber", required = false) Integer pageNumber,
			// @ApiParam(value = "pageSize 每页数据的数目") @RequestParam(value =
		...
		List<String> roles = dto.getRoleNames();
		if (roles.contains(RoleCode.ROLE_STUDENT)) {
			StuInforDomain stuInfor = authUtilService
					.getMentorInfo(dto.getId());
			mentorId = stuInfor.getMentorId();
		}

		PageData pageData = authUtilService.getStudentInfo(mentorId,
	...
	public ResponseEntity<Map<String, Object>> isAssignMentor(
		...
		List<String> roles = dto.getRoleNames();
		if (roles.contains(RoleCode.ROLE_STUDENT)) {
			Long groupId = redisDataService.getStuGroupId(dto.getId());
			if (null != groupId) {
				result.put("groupId", groupId);
				status = true;
			} else {
				TrainingGroupInfoDTO groupDTO = orgMangerService
						.queryTrainGroupInfo(dto.getId());
//				TrainingGroupInfoDTO groupDTO = new TrainingGroupInfoDTO(246L,246L);
				if (null != groupDTO) {
					status = true;
					result.put("groupId", groupDTO.getId());
					CacheUserGroupThread thread = new CacheUserGroupThread(
							groupDTO.getId());
					thread.start();
				}
			}
	...
	class CacheUserGroupThread extends Thread {
		private Long groupId;

		public CacheUserGroupThread(Long groupId) {
			this.groupId = groupId;
		}

		public void run() {
			List<GroupStuDomain> stuList = queryService
					.getGroupStuValid(groupId);
			if(!stuList.isEmpty()){
				redisDataService.cacheStuGroupId(groupId, stuList);
			}
		}
	}
}

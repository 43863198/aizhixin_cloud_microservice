...
public class DDClient {
    ...
    public void updateStuCache(Long orgId, Set<Long> studentIds) {
        ...
        try {
            UriComponents uriComponents = UriComponentsBuilder.fromHttpUrl(diandianApi + "/api/web/v1/org/synData/updateStudentCache").queryParam("orgId", orgId).queryParam("studentIds", studentIds.toString()).build();
            restUtil.get(uriComponents.toString(), "111");
        } catch (Exception e) {
            logger.warn("updateStuCacheException", e);
        }
